...
<form @submit.prevent="handle_submit($event)">
    <div class="mb-3">
        ...
        <div class="d-flex-center-start">
            <select v-model="expense.icon" class="form-control" name="icon" id="icon" @click="icon_was_set=true">
                <option v-for="(icon,i) in icons" :key="i" :value="i">{{icon.icon}}</option>
            </select>
            <input v-model="expense.name" @keyup="!icon_was_set && handle_icon_update()" type="text" class="form-control" id="name" placeholder="Asado en lo de...">
        </div>
    ...
    <div class="mb-3">
        ...
        <div class="mt-1" v-if="expense.group.length">
            ...
            <button v-for="(member,m) in expense.group" :key="m" class="btn btn-sm btn-secondary mb-1 me-1" @click.prevent="expense.group.splice(m,1)">
    ...
    <button class="btn btn-primary btn-lg w-100 mt-3" ref="submitButton">Crear nuevo gasto compartido</button>
...
export default {
    ...
    mounted() {
        this.expense.icon = get_random(0, this.icons.length - 1)
        this.$forceUpdate()
        setTimeout(() => {
            this.expense.icon = get_random(0, this.icons.length - 1)
            this.$forceUpdate()
    ...
    methods: {
        add_member() {
            ...
            if (this.new_member && this.new_member.trim() && this.new_member.trim().length <= 16) {
                ...
                let exists = this.expense.group.some((m) => m == name_normalized)
                if (!exists) {
                    this.expense.group.push(name_normalized)
        ...
        handle_submit(event) {
            ...
            ['name','group'].forEach((prop) => {
                ...
                if (prop == 'name' && !has_text(this.expense.name)) {
                    this.error.name = 'Es necesario un nombre'
                    is_valid = false;
                } else if (prop == 'group' && this.expense.group.length < 2) {
            ...
            // Set the string icon
            this.expense.icon = this.icons[this.expense.icon].icon;
            // add timestamp
            this.expense.timestamp = Date.now()
        ...
        handle_icon_update() {
            ...
            setTimeout(() => {
                // console.log("this.last_search_query","!=","this.expense.name",this.last_search_query!=this.expense.name);
                if (this.last_search_query != this.expense.name) {
                    this.last_search_query = this.expense.name
                    const match_icon = this.set_icon(icons, this.expense.name)
                    if (match_icon != null) this.expense.icon = match_icon.index
                }
            }, 400);
        },
